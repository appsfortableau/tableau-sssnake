...
class D3Renderer implements Renderer {
  ...
  constructor() {
    this.container = document.getElementById('scatter-plot');
    this.container?.insertAdjacentHTML('afterend', d3StartScreen);
  }
  screenGameStart() {}
  startGame() {}
  stopGame() {}
  ...
  init(game: Game) {
    ...
    axis.select('.x-axis .tick:first-child text').attr('transform', 'translate(-10, 5)');
    ...
    if (game.snake) {
      ...
      groupSnake
        ...
        .attr(
          'transform',
          `translate(${this.x(snakePath[0][0])}, ${this.y(snakePath[0][1])}) rotate(${game.snake.eyesRotation()})`
        )
  ...
  render(frame: Frame) {
    ...
    if (frame.snake) {
      ...
      snake
        ...
        .attr(
          'transform',
          `translate(${this.x(snakePath[0][0])}, ${this.y(snakePath[0][1])}) rotate(${frame.snake.eyesRotation()})`
        )
        .selectAll('circle')
    ...
    food
      ...
      .on('click', (e: MouseEvent) => {
        ...
        this.selection.forEach((tuple: number) => {
          this.d3.select(`.food-elm[data-tuple='${tuple}']`).classed('active', true);
        });

        this.d3.select('.food-group').classed('has-selection', this.selection.length > 0);
  ...
  toggleSelection(tupleId: number, mouseEvent: MouseEvent): number[] {
    ...
    const option = ctrlKeyPressed ? tableau.SelectOptions.Toggle : tableau.SelectOptions.Simple;
...
<div class="screen-form new-game" style="display: none">
  <h1>Game unlocked!</h1>
  <button class="btn btn-primary" data-mode="restart">
	START GAME
  </button>
</div>

<div class="screen-form continue-game" style="display: none">
  <h1>HUNGRY SNAKE</h1>
  <div class="group">
	<button class="btn btn-primary" data-mode="continue">
	  CONTINUE GAME
	</button>
	<button class="btn btn-secondary" data-mode="restart">
	  RESTART GAME
	</button>
  </div>
</div>
