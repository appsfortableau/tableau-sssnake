...
import { DataTable, SelectOptions, Tableau, Worksheet } from './types/extensions-api-types';

export type HoverDatapointCallback = (data: any, x: number, y: number) => void;
export type ClickDatapointCallback = (data: any, mouseEvent: MouseEvent) => void;
...
class Demo {
  ...
  async setData(worksheet: Worksheet, _: DataTable) {
  ...
  toggleSelection(selection: number[], tupleId: number, option: SelectOptions): number[] {
    switch (option) {
